...
function Job() {        
    ...
    const getISODate = (date) => {
        console.log('>>>>> inside getIsoDate date is: ', date);
        let dd = date.getDate();
        let mm = date.getMonth();
        let yyyy = date.getFullYear();

        console.log('>>>>> dd is: ', dd, '; mm is: ', mm, '; yyyy is: ', yyyy);
        console.log('>>>>> mm length is: ', mm.length);
    ...
    useEffect(() => {
        console.log('>>>>>>> 5. postJob useEffect');
        const postJob = async() => {
            console.log('>>>>>>>>> line no: 121: postJob method');
            let options = {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                ...
                body: JSON.stringify({
                    ...
                    lastDate: getISODate(lastDate)
                })
            };
            console.log('>>>> options= ', options);
            const response = await fetch('http://localhost:8080/api/post-job', options);
        ...
        if(isSubmitted){
            ...
            console.log('>>>> selectedEmploymentTypeOption is = ', selectedEmploymentTypeOption);
            console.log('>>>> selectedEmploymentTypeOptionId = ', selectedEmploymentTypeOption.id);
            console.log('>>> selectedExperienceLevelOption is = ', selectedExperienceLevelOption);
            console.log('>>> selectedExperienceLevelOptionId = ', selectedExperienceLevelOption.id);
            postJob();
        }
        // eslint-disable-next-line
    }, [isSubmitted]);
